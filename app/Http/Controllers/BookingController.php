...
class BookingController extends Controller
{
  ...
  public function store(Request $request)
  {
    ...
    Mail::send(
      'emails.booking',
      array('name' => $name, 'email' => $email, 'resource' => $resource, 'start' => $start, 'end' => $end, 'approve' => $approve, 'deny' => $deny, 'space' => $space, 'contact' => $contact),
  ...
  public function approve($token)
  {
    ...
    if (!empty($booking)) {
      if (!$booking->token == 0) {
        ...
        Mail::send(
          'emails.bookingApprove',
          array('space' => $space, 'booking' => $booking),
