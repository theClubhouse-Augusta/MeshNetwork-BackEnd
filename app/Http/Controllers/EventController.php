...
class EventController extends Controller
{
  ...
  public function calendarInvite(Request $request)
  {

    $validInput = $this->inputValidator->validateTestCal($request);
    if ($validInput['isValid']) {
      $start = $request['start'];
      $end = $request['end'];
      $description = strip_tags($request['description']);
      $title = $request['title'];
      $from = DateTime::createFromFormat('Y-m-d H:i:s', $start);
      $to = DateTime::createFromFormat('Y-m-d H:i:s', $end);
      $link = Link::create($title, $from, $to)->description($description);
